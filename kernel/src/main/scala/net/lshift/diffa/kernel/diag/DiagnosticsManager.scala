...
trait DiagnosticsManager {
   ...
   */
  def logPairEvent(scanId:Option[Long] = None, pair:PairRef, level:DiagnosticLevel, msg:String)
   ...
   */
  def checkpointExplanations(scanId:Option[Long] = None, pair:PairRef)
   ...
   */
  def logPairExplanation(scanId:Option[Long] = None, pair:PairRef, source:String, msg:String)
   ...
   */
  def writePairExplanationObject(scanId:Option[Long] = None, pair:PairRef, source:String, objName: String, f:OutputStream => Unit)

  /**
   * Queries for known events about the given pair.
   */
  def queryEvents(pair:PairRef, maxEvents:Int):Seq[PairEvent]

  /**
   * Retrieves the scan states for each pair configured within the given domain.
   */
  def retrievePairScanStatesForDomain(space:Long):Map[String, PairScanState]
